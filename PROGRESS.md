...
### Pembaruan 28 Agustus 2026 — Backup, pemulihan, dan rollback

- Toolkit manual tersedia di `deploy/backup/`: PowerShell untuk localhost Windows, Bash untuk SSH Linux, preflight read-only, dump MySQL/MariaDB, backup unggahan/konfigurasi/build/source Git, manifest checksum, verifikasi dan ekstraksi terisolasi.
- Snapshot membedakan label `local` dan `hostinger-testing`, wajib berada di luar project/webroot, memakai izin privat, dan menolak config cache/environment yang berbeda dari `.env`. Password tidak masuk argument/history; file credential sementara harus terhapus sebelum marker `COMPLETE` diterbitkan.
- Setelah tahap persiapan, snapshot yang gagal/terputus ditandai `INCOMPLETE` dan ditolak untuk restore. Kegagalan lebih awal (misalnya pengaturan izin) bisa hanya meninggalkan folder kosong, yang juga bukan backup valid. Tidak ada penghapusan backup lama, cron, maintenance mode, migration, seeder, impor SQL, atau overwrite aplikasi/database aktif otomatis.
- Runbook `deploy/BACKUP-PEMULIHAN-ROLLBACK.md` memisahkan rollback kode dari pemulihan data, mewajibkan database/folder baru untuk latihan, mempertahankan APP_KEY, memasangkan database dan unggahan dari waktu yang sama, serta mencatat bukti dan durasi pemulihan.
- Panduan Hostinger diperbarui untuk kondisi testing yang sudah berisi data. Langkah lama yang mengasumsikan database/webroot kosong dihapus; backup wajib sebelum pull/migration, sedangkan seeder/key generation/reset database bukan langkah update rutin.
- Verifikasi lokal 28 Agustus: 25 test / 134 assertion lulus pada Windows PowerShell 5.1 dan PowerShell 7. Drill memakai dua schema MySQL bernama acak, repository/file/data sintetis (Unicode, binary, foreign key), akun restore acak yang hanya punya hak ke schema tujuan, dump dan ekstraksi aktual, lalu impor ke schema baru. Ia juga membuktikan run tanpa konfirmasi ditolak, payload/manifest rusak ditolak, path/ZIP berbahaya ditolak, target berisi data tidak ditimpa, credential sementara dibersihkan ketika dump sengaja gagal, dan source DB tetap tidak berubah. Seluruh objek sintetis dihapus oleh test.
- Syntax Bash sudah diperiksa, tetapi backup nyata melalui SSH Hostinger **belum dijalankan**. Hasil backup dan latihan localhost dicatat pada pembaruan di bawah; salinan off-host dan kesiapan pemulihan Hostinger belum terbukti. RPO/RTO belum boleh diklaim terjamin.
- Preflight read-only terhadap localhost asli lulus pada 28 Agustus: toolkit memilih database `jcorp_company_profile`, repository root, SHA `0ecb4d4`, serta lokasi calon backup di luar project. Mode ini tidak membuat snapshot dan tidak mengubah data/cache.
- Percobaan backup asli oleh pemilik pada 28 Agustus berhenti saat mengatur ACL tujuan `D:\JCorp-Backups`; pemeriksaan read-only memastikan folder percobaan masih kosong. Penyebab direproduksi pada fixture temporer: helper awal memanggil `SetOwner`, padahal folder yang hanya mewarisi `Modify` tidak memberi hak `WRITE_OWNER`, meskipun pembuat sudah menjadi pemilik. Helper diperbaiki untuk hanya mengubah DACL lalu memverifikasi aturan privat sebelum menyalin credential/data; tidak ada fallback izin publik atau kebutuhan Administrator.
- Verifikasi perbaikan ACL: 25 test / 94 assertion lulus pada masing-masing Windows PowerShell 5.1 dan PowerShell 7, dengan 1 drill MySQL opt-in dilewati. Regresi awal memunculkan error yang sama sebelum perbaikan; setelahnya membuktikan izin hanya akun pembuat sampai berkas/subfolder, pemilik/grup serta izin parent tidak berubah, dan target lama tidak ditimpa. Test ini tidak mengakses database aplikasi. Setelah perbaikan, pemilik berhasil mengulang backup asli sebagaimana hasil berikut.

#### Hasil backup asli dan latihan pemulihan localhost — 28 Agustus 2026

- Snapshot asli `jcorp-snapshot-20260828T041430Z-2a8be059` berhasil dibuat pemilik di `D:\JCorp-Backups\local`. Marker `COMPLETE`, hash payload, inventaris ZIP, dan integritasnya diperiksa ulang. Snapshot tetap utuh, belum dienkripsi, dan sumber Git-nya SHA `0ecb4d4fce4cc6fb954b81c4acff4dc8c23318f3`.
- Pemilik memilih **backup lokal di D saja untuk sementara**. Tidak ada upload cloud/salinan off-host. Ini keputusan cakupan, bukan bukti terlindungi dari kerusakan drive atau kehilangan laptop.
- Dengan persetujuan pemilik, SQL dipulihkan ke database baru `jcorp_restore_20260828_01`, menggunakan pengguna MySQL baru yang dibatasi ke database itu. Akses pengguna latihan ke tabel database asli diuji dan ditolak. Nama database pada grant diperlakukan literal, bukan wildcard.
- Seluruh **12 tabel** dan jumlah baris cocok dengan snapshot: 6 usaha, 20 item katalog, 5 entri galeri, 2 akun, serta tabel pendukung. Waktu impor dan pemeriksaan DB pada skrip 2,19 detik; ini **bukan RTO aplikasi keseluruhan**. Hash isi lima tabel inti database asli serta `.env` asli tidak berubah selama proses impor.
- Pemulihan file dilakukan ke `D:\JCorp-Recovery\latihan-20260828-01`, bukan checkout utama. **157 file** snapshot dan **410 file** source diperiksa checksum-nya; aplikasi terpisah berada di subfolder `application`. Seluruh 26 rujukan gambar/thumbnail ditemukan, dan tidak ada referensi usaha yatim pada katalog, galeri, atau akun.
- Dependensi aplikasi latihan dipasang dari `composer.lock` dengan `--no-dev --no-scripts --no-plugins`; tidak memakai `composer setup`, migration, seeder, atau `key:generate`. APP_KEY snapshot dipertahankan. DB/user, cookie, session/cache/storage diarahkan ke latihan; mail memakai transport `array`, SSR dimatikan **hanya pada salinan latihan**, worker/scheduler tidak dijalankan.
- Server latihan hanya mendengarkan `127.0.0.1:8018`, document root `application/public`. Homepage, kelima profil, Ayodya EN, dan login memberi HTTP 200. **147 aset** (build, logo, gambar, unggahan) diminta lewat HTTP dan hash respons cocok. Path `.env`, `.git/config`, `composer.json`, dan log tidak terlayani (403/404).
- Browser membuka keenam halaman publik dan halaman login. Tombol Ayodya ID → EN diuji dan isi profil layanan tetap muncul dalam bahasa Inggris. Pemeriksaan ini bukan audit ulang semua ukuran layar/interaksi.
- Pemilik berhasil login sebagai **super admin** dengan akun yang ikut snapshot. Sesi tersebut diperiksa lewat browser: dashboard, pemilihan usaha, katalog lima unit (2 Sweetness, 1 Nails, 15 ngelash, 1 Ayodya, 1 J-Land), galeri Nails (4 foto)/ngelash (1 foto), form Kontak & Label Nails, daftar 6 usaha, serta daftar 2 akun bisa dibuka. Foto galeri lama termuat; ini belum membuktikan unggahan baru atau batas akses admin anak usaha.
- Uji browser katalog memakai satu item sintetis tersembunyi `LATIHAN-RESTORE-20260828` **hanya di database latihan**: tambah, baca kembali, ubah harga/deskripsi, cek tidak tampil di halaman publik, konfirmasi hapus, lalu soft delete berhasil. Baris uji ID 49 tanpa gambar dibersihkan secara terarah setelah soft delete terverifikasi. Tidak ada konten client yang diedit/dihapus. Hash isi tabel usaha, katalog, galeri, migration, identitas/status akun, serta `.env` sumber tetap sama; tabel latihan yang diperiksa kembali sama dengan sebelum uji. Session/cache dan counter auto-increment bukan bagian pembandingan ini.
- Uji unggah galeri: pemilik menambahkan `LATIHAN-RESTORE-FOTO` pada Nails melalui panel latihan (ID 13). File WebP 554 × 554 (65.346 byte) dan thumbnail 400 × 400 (28.668 byte) tersedia; keduanya memberi HTTP 200 dengan checksum respons sesuai file. Thumbnail termuat di panel dan galeri publik. Seluruh 23 berkas unggahan lama tetap cocok dengan snapshot; tabel usaha/katalog/galeri/migration sesuai sumber setelah mengecualikan satu foto uji. Foto uji tidak ada pada database/storage asli.
- Pemilik kemudian menghapus foto uji melalui panel. Pemeriksaan read-only mengonfirmasi `deleted_at` terisi pada ID 13, foto uji tidak tampil di panel maupun galeri publik, dan empat foto lama Nails tetap termuat serta ID-nya cocok dengan sumber. File gambar dan thumbnail uji masih ada dengan checksum sama seperti saat unggah, sesuai soft delete. Tidak ada penghapusan permanen atau pengaktifan kembali foto uji; record/file sengaja dipertahankan untuk pemulihan. Mengaktifkan kembali foto terhapus belum diuji.
- Pemilik berhasil login sebagai **admin Sweetness Things** memakai akun lama; identitas sesi dikonfirmasi lewat menu akun. Dashboard menampilkan 2 katalog, tanpa pemilih usaha, menu Manajemen, atau tautan Galeri. Katalog dan form kontak tetap milik Sweetness ketika parameter `?business=` diarahkan ke Nails/ngelash. Membuka `/jcorp-panel/users` dan `/jcorp-panel/businesses` langsung menampilkan 403; `/jcorp-panel/portfolio?business=nails-by-me` menampilkan 404. Pemeriksaan ini hanya navigasi GET dan pembacaan tampilan, bukan uji seluruh otorisasi POST/PUT/DELETE. Tidak ada perubahan konten/akun melalui tahap ini.
- **Uji dasar pemulihan lokal lulus** untuk database/file, halaman publik, login kedua peran, CRUD katalog sintetis, unggah/soft delete foto, dan batas akses baca yang dicoba. **Belum diuji:** penggantian gambar dan pengaktifan kembali satu item soft-deleted, otorisasi tulis admin anak usaha pada aplikasi latihan, penyimpanan kontak/penerbitan/undangan; pemulihan/backup Hostinger; dan rollback rilis penuh. Tidak ada reset password, akun aplikasi baru, commit, push, atau deployment dilakukan. Dirty working tree toolkit/dokumentasi/test tetap belum masuk `source.zip`; toolkit perlu di-commit sebelum dijadikan acuan rollback kode.
- Laporan privat: `D:\JCorp-Recovery\latihan-20260828-01\HASIL-LATIHAN.md` dan `restore-state.json`. Folder pemulihan juga mengandung konfigurasi/data akun, sehingga tidak boleh diunggah ke Git/webroot/chat.

#### Pemeriksaan pra-commit toolkit — 28 Agustus 2026

- Kandidat commit diperiksa: 16 file (5 tracked berubah + 11 file baru), hanya toolkit backup/ekstraksi, test, aturan ignore, dan dokumentasi. Tidak ada perubahan kode fitur website, migration, dependensi, atau build frontend; build ulang tidak diperlukan untuk perubahan ini.
